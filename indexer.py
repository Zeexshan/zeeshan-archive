...
def clean_filename(filename: str) -> str:
    ...
    filename = re.sub(r'\.(mkv|mp4|avi|mov|wmv|flv|webm|m4v)$',
                      '',
                      filename,
                      flags=re.IGNORECASE)

    # Try to find season/episode pattern
    season_episode_match = re.search(r'[.\s_-](S\d{1,2}E\d{1,2})[.\s_-]?',
    ...
    if season_episode_match:
        ...
        # Capitalize properly
        series_name = ' '.join(word.capitalize()
    ...
    async with app:
        ...
        async for message in app.get_chat_history(CHANNEL_ID):
            ...
            if message_count % 100 == 0:
                print(
                    f"Processed {message_count} messages, found {media_count} media files..."
                )
